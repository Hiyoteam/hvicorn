...
def plugin_init(
    ...
    def mark_afk(ctx: hvicorn.CommandContext):
        ...
        afked_users.update({ctx.sender.nick: [reason, time.time()]})
    ...
    def back_check(bot: hvicorn.Bot, event):
        ...
        if event.nick in afked_users.keys():
            if time.time() - afked_users[event.nick][1] < 0.1:
                return
    ...
    def on_chat(bot: hvicorn.Bot, event: hvicorn.ChatPackage):
        for user in afked_users.items():
            if f"@{user[0]}" in event.text:
                bot.send_message(f"@{event.nick} {afk_tip.format(nick=user[0])}"+(" "+reason.format(reason=user[1][0]) if user[1][0] else ""))
