...
from hvicorn.models.client import JoinRequest, ChatRequest, Message, UpdateMessageRequest
...
class Bot:
    ...
    def _connect(self) -> None:
        ...
        while not self.websocket.connected:
            sleep(1)
    def join(self) -> None:
        self._send_model(JoinRequest(nick=self.nick, channel=self.channel, password=self.password))
        sleep(1)
    def send_message(self, text, editable=False) -> Message:
        ...
        def wrapper(mode: Literal["overwrite", "prepend", "append", "complete"], text: str):
            self._send_model(UpdateMessageRequest(customId=msg.customId, mode=mode, text=text))
    ...
    def run(self, ignore_self:bool = True) -> None:
        ...
        while not self.killed:
            ...
            try:
                ...
                warn(f"Failed to parse event, ignoring: {package} cause exception: \n{format_exc()}")
                continue
